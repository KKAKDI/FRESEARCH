...
CREATE TABLE SUBJECT (
	SUBJ_CODE	VARCHAR2(10)	NOT NULL,	 -- 주제코드
	CTGR_CODE1	VARCHAR2(10)	NULL,		 -- 카테고리코드1
	CTGR_CODE2      VARCHAR2(10)	NULL,		 -- 카테고리코드2
	CTGR_CODE3	VARCHAR2(10)	NULL,		 -- 카테고리코드3
	MB_EMAIL	VARCHAR2(40)	UNIQUE NOT NULL, -- 이메일
	MB_NICK		VARCHAR2(24)	UNIQUE NOT NULL, -- 닉네임
	SUBJ_NM		VARCHAR2(240)   NULL,		 -- 주제명
	SUBJ_REGDATE	DATE		DEFAULT SYSDATE  -- 등록일
);
ALTER TABLE SUBJECT ADD CONSTRAINT PK_SUBJECT PRIMARY KEY (SUBJ_CODE);
ALTER TABLE SUBJECT ADD CONSTRAINT FK_CATEGORY_TO_SUBJECT FOREIGN KEY (CTGR_CODE1, CTGR_CODE2, CTGR_CODE3) REFERENCES CATEGORY (CTGR_CODE) ON DELETE CASCADE;

-- 질문
CREATE TABLE QUESTION (
	QST_CODE	VARCHAR2(10)    NOT NULL,	-- 질문코드
	SUBJ_CODE	VARCHAR2(10)	NULL,		-- 주제코드
	...
	QST_REGDATE	DATE		DEFAULT SYSDATE -- 등록일
);
...
-- 아이템
CREATE TABLE ITEM (
	ITEM_CODE	VARCHAR2(10)	NOT NULL,	-- 아이템코드
	QST_CODE	VARCHAR2(10)	NULL,		-- 질문코드
	ITEM_CONTENT    VARCHAR2(1000)  NULL,		-- 아이템내용
        ITEM_IMG	VARCHAR2(50)	NULL,		-- 이미지
	ITEM_REGDATE	DATE		DEFAULT SYSDATE -- 등록일
);
...
-- 답변
CREATE TABLE ANSWER (
	ASW_CODE	VARCHAR2(10)	NOT NULL,	 -- 답변코드
	ITEM_CODE	VARCHAR2(10)	NULL,		 -- 아이템코드
        MB_EMAIL	VARCHAR2(40)	UNIQUE NOT NULL, -- 이메일
	MB_NICK		VARCHAR2(24)	UNIQUE NOT NULL, -- 닉네임
	ASW_CONTENT	VARCHAR2(1000)	NULL,		 -- 답변내용
	ASW_REGDATE	DATE		DEFAULT SYSDATE  -- 등록일
);
...
-- 알림
CREATE TABLE NOTICE (
	NTC_CODE	    VARCHAR2(10)    NOT NULL, -- 알림코드
	MB_EMAIL	    VARCHAR2(40)    NULL,     -- 이메일
	MB_ATT_CTGR_CODE    VARCHAR2(10)    NULL,     -- 관심카테고리코드
...
-- 새소식
CREATE TABLE NEWS (
	NEWS_CODE           VARCHAR2(10)     NOT NULL,       -- 새소식코드
...
-- 자유게시판
CREATE TABLE BOARD (
	BRD_CODE	  VARCHAR2(10)	   NOT NULL,	    -- 게시판코드
	MB_EMAIL	  VARCHAR2(40)	   UNIQUE NOT NULL, -- 이메일
	MB_NICK		  VARCHAR2(24)     UNIQUE NOT NULL, -- 닉네임
	...
	BRD_REGDATE       DATE             DEFAULT SYSDATE  -- 등록일
);
ALTER TABLE BOARD ADD CONSTRAINT PK_BOARD PRIMARY KEY (BRD_CODE);

-- 댓글
CREATE TABLE REPLY (
	RPL_CODE	  VARCHAR2(10)    NOT NULL,	   -- 댓글코드
	BRD_CODE	  VARCHAR2(10)    NULL,		   -- 게시판코드
	MB_EMAIL	  VARCHAR2(40)    UNIQUE NOT NULL, -- 이메일
	MB_NICK		  VARCHAR2(24)    UNIQUE NOT NULL, -- 닉네임
	RPL_CONTENT	  VARCHAR2(1000)  NULL,		   -- 내용
	RPL_IMG		  VARCHAR2(50)    NULL,		   -- 이미지
	RPL_LIKE_CNT      NUMBER	  DEFAULT 0,       -- 추천수
	RPL_DISLIKE_CNT   NUMBER	  DEFAULT 0,       -- 비추천수
	RPL_REGDATE	  DATE            DEFAULT SYSDATE  -- 등록일
);
...
CREATE SEQUENCE ATT_CATEGORY_SEQ
INCREMENT BY 1
START WITH 1
nocache;
